...
class Category extends Model
{
    ...
    public static function getCategory(int $categoryId): object|null
    {
        ...
        if(!empty($category))
        {
            ...
            $category->city = self::where('detsad_addresses', '>', '100')
                ->where('id', $city_id)
                ->first();
        }
        return $category;
    }
    public static function getDistrict(int $categoryId, string $district): object|null
    {
    ...
    public static function getDistricts(int $categoryId): Collection
    {
        return DB::table('i1il4_detsad_districts as t1')
    ...
    public static function getItems(int $categoryId, string $district = ''): Collection
    {
        return DB::table('i1il4_detsad_items as t1')
            ...
            ->when(!empty($district), function ($query) use ($district) {
                $query->where('t2.district', $district);
            })
            ->get();
    }
}
